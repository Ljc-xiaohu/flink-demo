 ...
 * 分时/分级窗口和之前的秒级窗口类似,也是把当前窗口最新的CleanBean转为IndexBean并收集
 * 但是分时窗口的IndexBea中需要计算最近一分钟内的当前指数的成交量和成交金额
...
public class IndexMinutesWindowFunction extends RichWindowFunction<CleanBean, IndexBean, String, TimeWindow> {
    //开发步骤：
    //1.定义MapState<股票代码, IndexBean>用来缓存上一窗口的数据
    MapState<String, IndexBean> indexState = null;

    //2.初始化MapState
    @Override
    public void open(Configuration parameters) throws Exception {
        //定义状态描述器
        MapStateDescriptor<String, IndexBean> stateDescriptor = new MapStateDescriptor<>("indexMs", String.class, IndexBean.class);
        //根据状态描述器初始化状态
    ...
    public void apply(String s, TimeWindow timeWindow, Iterable<CleanBean> iterable, Collector<IndexBean> collector) throws Exception {
        ...
        indexBean.setTradeVol(minutesVol);
        indexBean.setTradeAmt(minutesAmt);//秒级行情不需要计算最近5s有多少成交量和成交金额,分时行情才需要
