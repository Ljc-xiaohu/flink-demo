...
public class StockSecondsTask implements ProcessDataInterface {
    ...
    public void process(DataStream<CleanBean> waterData) {
        ...
        waterData.keyBy(CleanBean::getSecCode)//按照个股代码分组"secCode"
        //TODO 2.划分时间窗口.timeWindow(Time.seconds(5))//5s一个窗口，不重叠的时间窗口
        .timeWindow(Time.seconds(5))
        //TODO 3.秒级窗口函数业务处理.apply(new StockSecondsWindowFunction())
        //如果是简单业务可以直接使用sum/min/max...
        //如果是复杂业务可以apply或process
        //下面的apply完成了将最近5s内最新的CleanBean封装为了StockBean
        .apply(new StockSecondsWindowFunction())
        //TODO 4.窗口数据合并.timeWindowAll(Time.seconds(5))
        .timeWindowAll(Time.seconds(5))// 把所有个股5s内最新的StockBean合到一个窗口
        //TODO 5.封装List<Put>批写入对象.apply(new StockPutHBaseWindowFunction())
        .apply(new StockPutHBaseWindowFunction())
        //代码走到这里数据已经封装到List<Put>中的
        //TODO 6.数据写入.addSink(new HBaseSink(QuotConfig.STOCK_HBASE_TABLE_NAME));
       .addSink(new HBaseSink(QuotConfig.STOCK_HBASE_TABLE_NAME));
    }
}
