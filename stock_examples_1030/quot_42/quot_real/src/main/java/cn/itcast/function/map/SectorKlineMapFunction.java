...
public class SectorKlineMapFunction extends RichMapFunction<SectorBean, Row> {
    ...
    public SectorKlineMapFunction(String type, String firstTradeDateName) {
        this.type = type;
 ...
 */
        String aggSQL = "SELECT\n" +
                ...
                "GROUP BY\n" +
                "    sector_code";
        aggMap = DBUtil.query("sector_code", aggSQL);
    }

    //二.数据转换sectorBean-->Row
    @Override
    public Row map(SectorBean sectorBean) throws Exception {
        //开发步骤：
        //1.获取当前板块数据:前收、收、开、高、低、成交量、成交金额
        String sectorCode = sectorBean.getSectorCode();
        ...
        //如果是日K,那么前收、收、开、高、低、成交量、成交金额就直接存入日K表即可,因为sectorBean就是当前今天最新的行情!
        //如果是周K或月K,那么前收、收、开、高、低、成交量、成交金额需要把sectorBean中的数据和aggMap中的合并
        ...
        if (firstTradeDateTime < tradeDateTime && (type.equals(2) || type.equals(3))) {
            ...
            //trade_vol,
            //trade_amt
            Map<String, Object> map = aggMap.get(sectorCode);
        ...
        //如果不走if肯定是日K,日直接使用sectorBean中的数据
