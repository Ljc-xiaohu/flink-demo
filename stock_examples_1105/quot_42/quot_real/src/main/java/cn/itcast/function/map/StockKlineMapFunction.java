...
public class StockKlineMapFunction extends RichMapFunction<StockBean, Row> {
    ...
    public StockKlineMapFunction(String type, String firstTradeDateName) {
        this.type = type;
 ...
 */
    ...
    public Row map(StockBean stockBean) throws Exception {
        //开发步骤：
        //1.获取当前个股数据:前收、收、开、高、低、成交量、成交金额
        ...
        //如果是日K,那么前收、收、开、高、低、成交量、成交金额就直接存入日K表即可,因为stockBean就是当前今天最新的行情!
        //如果是周K或月K,那么前收、收、开、高、低、成交量、成交金额需要把stockBean中的数据和aggMap中的合并
        ...
        //如果不走if肯定是日K,日直接使用stockBean中的数据
